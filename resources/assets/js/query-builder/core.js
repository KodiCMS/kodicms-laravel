...
QueryBuilder.prototype.createRuleOperators = function(rule) {
    ...
    if (!$operatorSelect.is(':input')) {
        $operatorSelect.find(':input').on('change', function() {});
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    rule.field =  getQueryBuildField(rule);

    var ruleInputs = rule.field.getInputs();
    for(i in ruleInputs) {
        $valueContainer.append(ruleInputs[i]);
        $inputs = $inputs.add(ruleInputs[i]);
    }
...
QueryBuilder.prototype.updateRuleOperator = function(rule, previousOperator) {
    ...
    if (!rule.operator || rule.operator.nb_inputs === 0) {
        ...
        if ($valueContainer.is(':empty') || rule.operator.nb_inputs !== previousOperator.nb_inputs || rule.operator.multiple !== previousOperator.multiple) {
            this.createRuleInput(rule);
        }
    }
