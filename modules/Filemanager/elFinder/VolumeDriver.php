...
abstract class VolumeDriver
{
    ...
    public function mount(array $opts)
    {
        ...
        $this->disabled = isset( $this->options['disabled'] ) && is_array($this->options['disabled']) ? $this->options['disabled'] : [];
    ...
    public function options($hash)
    {
        return [
            ...
            'archivers'     => [
                ...
                'create'  => is_array($this->archivers['create']) ? array_keys($this->archivers['create']) : [],
                'extract' => is_array($this->archivers['extract']) ? array_keys($this->archivers['extract']) : [],
            ],
        ];
    }
    ...
    public function upload($fp, $dst, $name, $tmpname)
    {
        ...
        if ($mime == 'unknown' && $this->mimeDetect == 'internal') {
            $mime = VolumeDriver::mimetypeInternalDetect($name);
        }
    ...
    {
        return $this->commandDisabled('rm') ? [elFinder::ERROR_ACCESS_DENIED] : $this->remove($this->decode($hash));
    }
    ...
    protected function mimetype($path, $name = '')
    {
        ...
        if ($this->mimeDetect == 'finfo') {
            if ($type = @finfo_file($this->finfo, $path)) {
                ...
                if ($ext && preg_match('~^application/(?:octet-stream|(?:x-)?zip)~', $type)) {
                    if (isset( static::$mimetypes[$ext] )) {
                        $type = is_array(static::$mimetypes[$ext]) ? static::$mimetypes[$ext][0] : static::$mimetypes[$ext];
                    }
                }
            }
            ...
            $type = VolumeDriver::mimetypeInternalDetect($path);
        }
        ...
        return $type == 'unknown' && $this->mimeDetect != 'internal' ? VolumeDriver::mimetypeInternalDetect($path) : $type;
    ...
    static protected function mimetypeInternalDetect($path)
    {
        ...
        return isset( static::$mimetypes[$ext] ) ? is_array(static::$mimetypes[$ext]) ? static::$mimetypes[$ext][0] : static::$mimetypes[$ext] : 'unknown';
    ...
    protected function imgResize(
        $path, $width, $height, $keepProportions = false, $resizeByBiggerSide = true, $destformat = null
    ) {
    ...
    protected function imgSquareFit(
        $path, $width, $height, $align = 'center', $valign = 'middle', $bgcolor = '#0000ff', $destformat = null
    ) {
        if (( $s = @getimagesize($path) ) == false) {
            return false;
        }
