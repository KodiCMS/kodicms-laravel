<?php
return [
	'title' => [
		'information' => 'Информация о разделе',
		'section' => 'Разделы',
		'edit' => 'Редактирование раздела [:name]',
		'create' => 'Новый раздел',
		'create_document' => 'Новый документ',
		'edit_document' => 'Редактирование документа [:name]'
	],
	'information' => [
		'name' => 'Название',
		'description' => 'Описание',
		'show_in_root' => 'Показывать в корне меню',
		'icon' => 'Иконка',
		'menu_position' => 'Позиция'
	],
	'ordering' => [
		'title' => 'Сортировка документов',
		'order_by' => 'Сортировать по',
		'fields' => 'Доступные поля'
	],
	'button' => [
		'create' => 'Создать раздел',
		'create_folder' => 'Добавить папку',
		'remove_fields' => 'Удалить поля',
		'create_field' => 'Добавить поле'
	],
	'toolbar' => [
		'check_all' => 'Все',
		'uncheck_all' => 'Ни одного',
		'remove_selected' => 'Удалить выбранные',
		'create_document' => 'Создать документ'
	],
	'field' => [
		'information' => 'Информация',
		'list' => 'Список полей раздела',
		'position' => 'Позиция',
		'key' => 'Ключ',
		'name' => 'Название',
		'type' => 'Тип',
		'visible' => 'Отображать в списке',
		'settings' => 'Настройки',
		'hint' => 'Подсказка',
		'required' => 'Обязательно',
		'default_value' => 'Значение по умолчанию',
		'related_ds' => 'Связанный раздел',
		'related' => 'Связанные поля других разделов'
	],
	'messages' => [
		'empty' => 'В разделе нет документов',
		'document_updated' => 'Документ сохранен'
	]
];