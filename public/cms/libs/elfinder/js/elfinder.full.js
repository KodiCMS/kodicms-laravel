	...
	$.fn.elfindersortbutton = function (cmd) {
		...
		return this.each(function () {
			var fm = cmd.fm,
				...
				asc = selected + '-asc',
				desc = selected + '-desc',
				button = $(this).addClass('ui-state-default elfinder-button elfinder-menubutton btn elfiner-button-' + name)
					.attr('title', cmd.title)
				...
				if (mime.indexOf('text/') === 0 || $.inArray(mime, mimes) !== -1) {
					...
					jqxhr = fm.request({
						...
						.done(function (data) {
							...
							CMS.filters.switchOn('elfinder-quicklook-preview-text', 'ace');
						});
				}
			});
		},
